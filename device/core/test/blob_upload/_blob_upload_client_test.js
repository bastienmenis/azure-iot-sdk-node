...
describe('BlobUploadClient', function() {
  ...
  describe('#uploadToBlob', function() {
    ...
    /*Tests_SRS_NODE_DEVICE_BLOB_UPLOAD_CLIENT_16_007: [`uploadToBlob` shall call the `done` callback with a `BlobUploadNotificationError` if notifying the IoT Hub instance of the transfer outcome fails.]*/
    ...
    it('calls the done callback with BlobUploadNotificationError if notifying the IoT Hub instance of the transfer outcome fails', function(done) {
      ...
      fakeBlobUploaderFailure.uploadToBlob = function(blobInfo, stream, streamLength, callback) {
        callback(null, 'deviceId/' + fakeBlobName,  { statusCode: 400, body: 'Random Error Code that is not 2xx' });
      };

      var clientuploadsuccess = new BlobUploadClient(fakeConfig, fakeFileUpload, fakeBlobUploaderSuccess);
      var clientuploadfailure = new BlobUploadClient(fakeConfig, fakeFileUpload, fakeBlobUploaderFailure);
      clientuploadsuccess.uploadToBlob(fakeBlobName, fakeStream, fakeStreamLength, function(err1) {
        assert.strictEqual(err1.name, 'BlobUploadNotificationError');
        clientuploadsuccess.uploadToBlob(fakeBlobName, fakeStream, fakeStreamLength, function(err2) {
          assert.strictEqual(err2.name, 'BlobUploadNotificationError');
          done();
        });
      });
    });
    ...
    it.only('calls the done callback with a BlobUploadNotificationError if the data transfer fails', function(done) {
      var fakeStream = new stream.Readable();
      var fakeFileUpload = new FakeFileUploadApi();
      var fakeBlobUploader = new FakeBlobUploader();
      var fakeBlobName = 'blobName';
